...
void loop(void) 
{
 ...
 if ((count % 200) == 0) {
  ...
  // If the change in heading is negligible, do not move the servo.
  //if (headingDegrees - previousHeading > 1) {
    if (headingDegrees >= 270 || headingDegrees <= 90) {
      // If the heading is to the right, move to the left.
      if (headingDegrees < 90) {
        servo.write(90 - headingDegrees);
      }
      // If the heading is to the left, move to the right.
      else {
